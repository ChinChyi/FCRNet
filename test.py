...
DATASETS = ['./data_test/SOD', './data_test/PASCAL-S', './data_test/ECSSD', './data_test/HKU-IS',
            './data_test/DUT-OMRON', './data_test/DUTS']
...
class Test(object):
    ...
    def accuracy(self):
        with torch.no_grad():
            ...
            cost_time = 0
            for image, mask, coarse, (H, W), maskpathin in self.loader:
                image, mask            = image.cuda().float(), mask.cuda().float()
                start_time = time.time()
                out2, out3, out4, out5 = self.net(image, mask)
    ...
    def save(self):
        with torch.no_grad():
            for image, mask, coarse, (H, W), name in self.loader:
                out2, out3, out4, out5 = self.net(image.cuda().float(), mask.cuda().float())
