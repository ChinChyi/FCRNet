...
""" Fine & Coarse Refined Module"""
class FCRM(nn.Module):
    def __init__(self, cfg):
        super(FCRM, self).__init__()
        self.cfg     = cfg
        self.bkbone  = ResNet()
    ...
    def forward(self, x, coarse):
        ...
        ##print('out1=', out1.shape, 'out2=', out2.shape, 'out3=', out3.shape, 'out4=', out4.shape, 'out5=', out5_.shape,)
        #coarse . global context infornation
        if self.cfg.mode != 'test':
            coarse_size = out5_.shape[2]
            coarse = F.interpolate(coarse, size=[coarse_size, coarse_size])
            out5_ = torch.mul(out5_, coarse)
        # print('size=', coarse_size)

        # if self.cfg.mode == 'test':
        #     coarse = F.interpolate(coarse, size=[10, 10])
        # else:
        #     coarse = F.interpolate(coarse, size=[9, 9])
